...
По умолчанию пишет результаты в reports/runtime/test_results_runtime.json (UTF-8).
Для явного обновления корневого baseline-файла используйте --output test_results_runtime.json.
"""
...
DEFAULT_OUTPUT = REPO_DIR / "reports" / "runtime" / "test_results_runtime.json"

RESULTS = []


def parse_args():
    parser = argparse.ArgumentParser(description="Runtime Loader Test для модульных персон.")
    parser.add_argument(
        "--output",
        type=str,
        default=str(DEFAULT_OUTPUT),
        help="Путь к файлу результатов (по умолчанию: reports/runtime/test_results_runtime.json)",
    )
