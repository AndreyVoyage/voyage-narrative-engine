...
class TestArchitectureBoundary:
    ...
    def test_no_character_lab_or_companion_imports(self) -> None:
        """CRP_MAINLINE_CONSOLIDATION_V1 Part 5/19 -- the consolidated
        ``services/crp_authoring`` (including ``application_adapter.py``) must
        stay importable and testable before the Character Lab Foundation is
        published: it must never import the unpublished
        ``services.character_lab_application``/``ui.character_lab``, nor the
        superseded ``services.character_companion``/``character_core``/
        ``character_runtime``/legacy ``services.character_lab``."""
        forbidden = (
            ...
            "services.character_lab.",
        )
        violations = []
        for py_file in _list_prod_files():
            tree = ast.parse(py_file.read_text(encoding="utf-8"))
            for m in _modules(tree):
                if m == "services.character_lab" or any(m.startswith(f) for f in forbidden):
                    violations.append(f"{py_file}: imports {m}")
        assert not violations, f"Lab/Companion imports: {violations}"