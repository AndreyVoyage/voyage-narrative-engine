...
# -*- coding: utf-8 -*-
"""
Voyage Session Finalizer v2.0
Рефакторинг с исправлением критических проблем v1.0:
  - Динамическое обнаружение модулей (semver-авто)
  - Исправлен VSCNO инвариант (сумма = 10)
  - Graceful error handling (без sys.exit)
  - Полный memory update (trust/attraction)
  - Валидация state и persona
  - Улучшенный Narrative Editor (главы, сенсорика)
  - Улучшенный Visual Pipeline (приоритет по фазам)
  - CLI с --validate, --verbose, --dry-run
...
# Исправленные VSCNO defaults (инвариант: сумма = 10)
VSCNO_DEFAULTS = {
    "kira":   {"ВЛ": 2, "СТ": 3, "НЖ": 3, "ОГ": 2},  # 10
    "sergey": {"ВЛ": 2, "СТ": 2, "НЖ": 2, "ОГ": 4},  # 10
    "marina": {"ВЛ": 1, "СТ": 2, "НЖ": 4, "ОГ": 3},  # 10
    "maksim": {"ВЛ": 3, "СТ": 2, "НЖ": 2, "ОГ": 3},  # 10
}

# Подуровни и их speech-признаки (расширенные)
LEVEL_SIGNATURES = {
    "kira": {
...
# Lighting map по подуровням (расширен для Максима)
LIGHTING_MAP = {
...
# Сенсорные якоря для сценариев (улучшение Narrative Editor)
SCENARIO_SENSORY = {
    "sauna_quartet": {
...
def discover_persona(char_id: str) -> Optional[Path]:
    """Динамически находит последнюю версию модуля персонажа по semver."""
    ...
    if matches:
        path = Path(matches[0])
        debug(f"Discovered persona for {char_id}: {path.name}")
        return path
    # Fallback: поиск без версии
...
def load_persona(char_id: str) -> Optional[Dict]:
    """Загружает JSON-модуль персонажа (динамическое обнаружение)."""
    path = discover_persona(char_id)
    if not path or not path.exists():
        log(f"Persona not found for '{char_id}' (pattern: {char_id.upper()}_MODULE_v*.json)", "ERROR")
        return None
    try:
...
def save_persona(char_id: str, data: Dict):
    """Сохраняет JSON-модуль с бэкапом и валидацией."""
    path = discover_persona(char_id)
    if not path:
        # Создаём новый файл с базовым именем
...
def validate_vscno(vscno: Dict[str, int], char_id: str) -> bool:
    """Проверяет инвариант ВЛ+СТ+НЖ+ОГ = 10."""
...
def parse_log(log_path: Path) -> List[Dict]:
    """Парсит лог сессии на реплики. Поддерживает Unicode-варианты имён."""
    ...
    # Расширенные паттерны акторов (Unicode + ASCII)
    actor_patterns = [
    ...
    for line in lines:
        ...
        for pattern, actor_id in actor_patterns:
            if re.search(pattern, line, re.IGNORECASE):
                ...
                # Убираем префикс имени из текста
...
def detect_level_from_text(char_id: str, text: str) -> Optional[str]:
    """Определяет подуровень персонажа по тексту с confidence score."""
...
def detect_phase_from_text(text: str) -> Optional[str]:
    """Определяет фазу сценария из текста."""
    text_lower = text.lower()
    phases = {
...
class StateManager:
    def __init__(self, entries: List[Dict], scenario_id: str = "unknown"):
    ...
    def process(self) -> SessionState:
        ...
        log(f"[State Manager] Done. Turns: {self.state.turn_count}, Levels changed: {total_level_changes}, Phases: {self.state.phases_detected}")
        return self.state
    ...
    def _analyze_relationship_deltas(self, char_id: str, text: str, fmdr: Dict):
        """Анализирует изменения trust/attraction из текста."""
        text_lower = text.lower()
        ch = self.changes[char_id]
        # Trust signals
        trust_signals = [
    ...
    def _clamp_metrics(self, char_id: str):
        ...
        for key in ["desire", "anxiety", "desire_tension", "frustration"]:
            char.internal_state[key] = clamp_metric(
                char.internal_state[key] + ch[f"{key}_delta"]
            )
        # VSCNO invariant check + auto-fix
        total = sum(char.vscno.values())
        if total != 10:
            log(f"{char_id}: VSCNO auto-fixing sum={total} → 10", "WARN")
            diff = 10 - total
            # Распределяем разницу по ОГ (общность) как наиболее гибкая ось
            char.vscno["ОГ"] = clamp_metric(char.vscno.get("ОГ", 2) + diff)
            # Если всё ещё не 10 — распределяем пропорционально
            total2 = sum(char.vscno.values())
            if total2 != 10:
                factor = 10 / total2
                char.vscno = {k: max(0, min(4, round(v * factor))) for k, v in char.vscno.items()}
                # Корректируем округление
                final_total = sum(char.vscno.values())
                if final_total != 10:
                    char.vscno["ОГ"] += (10 - final_total)
        char.flags = list(set(self.changes[char_id]["flags"]))

    def generate_memory_update(self) -> List[Dict]:
        """Генерирует MEMORY_UPDATE.json с полными trust/attraction дельтами."""
        updates = []
        for char_id, char_state in self.state.characters.items():
            ...
            # Формируем trust/attraction updates
            trust_updates = {}
            for target, delta in ch["trust_deltas"].items():
                trust_updates[f"trust_levels.{target}"] = clamp_metric(
                    (char_state.internal_state.get("trust", 50) + delta)
                )
            attraction_updates = {}
            for target, delta in ch["attraction_deltas"].items():
                attraction_updates[f"attraction_levels.{target}"] = clamp_metric(
                    (char_state.internal_state.get("attraction", 50) + delta)
                )
    ...
    def to_dict(self) -> Dict:
        """Сериализует SessionState в dict."""
        return {
            "session_id": self.state.session_id,
...
class NarrativeEditor:
    ...
    def generate(self) -> str:
        ...
        # Story body — разбивка на главы по фазам
        ...
        # Добавляем сенсорные детали в начало каждой главы (если мало контента)
        ...
        for event in self.state.audit_log[:10]:  # Топ-10 событий
    ...
    def _vscno_to_prose(self, text: str) -> Optional[str]:
        """Превращает мнемоники атмосферы в прозу."""
        m = re.search(r"ВЛ(\d).*?СТ(\d).*?НЖ(\d).*?ОГ(\d)", text, re.IGNORECASE)
        if not m:
            return None
    ...
    def _user_to_prose(self, text: str) -> Optional[str]:
        ...
        if text.startswith("ВЛ") or text.startswith("СТ") or text.startswith("НЖ") or text.startswith("ОГ"):
            return None
        return f"Он {text}."
    ...
    def _fmdr_to_prose(self, actor: str, text: str) -> str:
        ...
        # If no FMDR found, treat as plain text (clean up technical prefixes)
        if not parts:
    ...
    def _inject_sensory(self, lines: List[str]) -> List[str]:
        """Добавляет сенсорные детали в начало первой главы."""
        if not self.sensory:
            return lines
        # Находим первую главу
...
class VisualPipeline:
    ...
    def extract(self) -> List[VisualMoment]:
        ...
        # Sort by score, but prioritize level transitions and key events
        self.moments.sort(key=lambda x: (x.score, x.is_level_transition, x.is_key_event), reverse=True)
        # Берём top 10, но гарантируем минимум 1 момент на фазу если есть
    ...
    def generate_prompts(self) -> str:
        ...
        for i, moment in enumerate(self.moments, 1):
            ...
            # Camera based on intimacy and phase
            ...
            # Negative prompt (персонаж-специфичный)
...
def main():
    parser = argparse.ArgumentParser(
        description="Voyage Session Finalizer v2.0 — один файл, весь pipeline",
...
Примеры:
  ...
  python session_finalize.py --log session.log --scenario shy_bloom --no-git --validate
        """
    )
    parser.add_argument("--log", required=True, help="Путь к логу сессии (.log или .txt)")
    parser.add_argument("--scenario", default="unknown", help="ID сценария (sauna_quartet, promenade, cafe_date, shy_bloom)")
    parser.add_argument("--repo", default=str(REPO_DIR), help="Путь к репозиторию")
    ...
    log(f"=== Voyage Session Finalizer v2.0 ===")
    log(f"Log: {log_path}")
    ...
    # 3. Load personas (динамическое обнаружение)
    ...
    # 6. Save outputs (или stdout в dry-run)
    ...
    if args.dry_run:
        ...
        for update in memory:
            ...
            if "trust_levels" in mem_changes and mem_changes["trust_levels"]:
                ...
                for key, value in mem_changes["trust_levels"].items():
                    # key может быть "trust_levels.user" или просто "user"
                    target = key.replace("trust_levels.", "")
                    persona["memory"]["trust_levels"][target] = clamp_metric(value)

            # Update attraction_levels
            if "attraction_levels" in mem_changes and mem_changes["attraction_levels"]:
                ...
                for key, value in mem_changes["attraction_levels"].items():
                    target = key.replace("attraction_levels.", "")
                    persona["memory"]["attraction_levels"][target] = clamp_metric(value)
