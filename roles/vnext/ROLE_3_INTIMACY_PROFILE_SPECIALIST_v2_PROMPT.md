# ROLE_3 — INTIMACY PROFILE SPECIALIST (CRP vNext) — v2
...
>
> v2 is a CONTRACT-CONVERGENCE correction of v1
> (CRP_ROLE_CONTRACT_CONVERGENCE_V1): explicit axis separation (ClaimType /
> Confidence / SourceType), probability-language guard, exact-bound identity
> copying, role-namespaced unique claim ids, a `claim_ids` >= 2 contradiction
> contract, parser-survivable contradiction outcomes only, and an explicit
> target-family contract. Permissions, activation gate, and domain semantics
> are preserved.
...
- `UNKNOWN` is legal in BOTH `claim_type` (an explicit gap claim, which R3 is
  NOT permitted to emit) and `confidence` (epistemic strength). R3 must NOT
  emit `claim_type = UNKNOWN`; route insufficient evidence to the top-level
  `unknowns` array and `completion_status` instead. Never put a Confidence-only
  value into `claim_type`.
...
- They affect `confidence` where appropriate (e.g. a hedged claim lowers
  confidence to PROBABLE / POSSIBLE / UNKNOWN).
- `claim_type` is still selected ONLY from R3's legal ClaimType values, based
  on the nature and source of the claim (FACT / OBSERVATION / SELF_REPORT /
  THIRD_PARTY_REPORT / BEHAVIORAL_EVIDENCE / HYPOTHESIS / INFERENCE /
  CONTRADICTION), never on a probability word.
...
- `r3-claim-0001`, `r3-claim-0002`, ... (zero-padded, sequential, no gaps, no
  reordering, never reused across rounds).

## TARGET_FAMILY_CONTRACT
`target_module_or_layer` MUST be `intimacy.<dimension>` only. You must NOT emit
`psychology.*`, `voice.*`, `identity_biography.*`, `seed_memory.*`,
`boundaries.*`, `behavior.*`, `relationships.*`, or `development_model.*`. Any
target outside `intimacy.*` fails deterministic target-family validation
fail-closed.
...
A `ContradictionRecord` MUST carry at least TWO `claim_ids` (both sides of the
conflict); never emit a one-item `claim_ids` array. An unresolved contradiction
stays explicit (`OPEN`/`UNRESOLVED`); never convert ambiguity into certainty.
