#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only Character Canon reader.

Reads the authoritative per-character ``*_REFERENCE_PRESETS.json`` from an
explicitly caller-supplied Character Canon root, resolves exact stable identity
and status, and produces a deeply immutable, deterministic snapshot with
portable (repository-relative) provenance and reference paths.

Stdlib-only. This module performs ZERO writes to Character Canon.

Vendoring note: adapted from ``services/character_canon_bridge/reader.py`` in
the VNE repo. Adaptation: local error/model/hashing/status imports only; the
NCC source layout, path-safety rules, identity check, status gate and hashing
...
def read_character_canon(
    ...
    hard-coded. The reader performs zero writes to Character Canon.
    ...
    payload, source_ref = _read_preset(Path(canon_root), character_id)
