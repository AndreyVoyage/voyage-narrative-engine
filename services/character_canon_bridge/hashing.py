...
# -*- coding: utf-8 -*-
"""
Deterministic content-hash helpers for Character Canon Read Bridge v0.
...
def compute_content_hash(payload: dict[str, Any]) -> str:
    """Compute the snapshot ``content_hash`` over the semantic payload.

    ``payload`` is produced by ``CharacterCanonSnapshot.semantic_payload()``
    and contains only the normalized Canon state (character/status/references
    and active_version). Envelope/source metadata (schema_version,
    provenance, content_hash) are excluded, and machine-specific roots never
    enter the payload.
    """
