...
## Управление рисками и верификацией (OD-GOV-RISK-01)

- Классификация риска (R0–R4) обязательна перед каждой ограниченной задачей.
- Единственный авторитетный источник политики: `governance/RISK_VERIFICATION_POLICY_v1.md`.
- Глубина верификации следует классу риска.
- Независимые аудиты — только при эскалации риска.
- Обнаружение более высокого риска → `STOP_AND_RETURN_TO_OWNER`.
- Без автоматического gate creep.

---

## Ветки, воркетри и публикация (OD-GOV-BRANCH-01)

- Единственный авторитетный источник политики: `governance/BRANCH_LINEAGE_AND_TASK_BATCHING_POLICY_v1.md`.
- Не дробить одну понятную задачу на микро-слайсы ради процесса; делить
  только на реальной архитектурной/риск-границе.
- Значимая task-ветка/worktree фиксирует свой base target/SHA и
  return/integration target.
- Расхождение архитектурной линии с целью интеграции → STOP, реконсиляция
  перед интеграцией, а не автоматический cherry-pick/rebase.
- Свежий pre-push `git ls-remote` обязателен, когда публикация в
  авторитетный ref зависит от ожидаемого target OID; дополняет, а не
  заменяет обязательную post-push проверку.
- Операционный branch/worktree registry ещё не построен — отдельная
  будущая задача `VOYAGE_BRANCH_WORKTREE_REGISTRY_V1`.
