...
def _run_renpy_v2_playable_exporter_tool(args: list[str]) -> int:
...
# ---------------------------------------------------------------------------
# Command: RenPy V2 playable exporter wrapper
# ---------------------------------------------------------------------------

def cmd_renpy_playable_v2(args: argparse.Namespace) -> int:
    print(f"=== RN RENPY PLAYABLE V2: {args.scene} -> {args.output} ===")
    return _run_renpy_v2_playable_exporter_tool(["build", args.scene, "--output", args.output])
...
def main() -> None:
    ...
    p_renpy_playable_v2 = sub.add_parser(
        "renpy-playable-v2",
        help="Build a playable RenPy scene from a V2 JSON source",
    )
    p_renpy_playable_v2.add_argument(
        "scene",
        help="Scene identifier or .v2.json path (e.g. SC_017)",
    )
    p_renpy_playable_v2.add_argument(
